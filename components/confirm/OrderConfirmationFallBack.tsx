  ...
  return (
    <div className="flex flex-col gap-4 bg-white p-5 rounded text-gray-400">
      ...
      {Array.from({ length: 3 }).map((_, idx: number) => {
        return (
          <div className="inline-flex p-3 border-2 border-gray-300 rounded gap-4 animate-pulse" key={idx}>
            <div className="relative w-20 h-20 rounded border-2 border-gray-300" key={idx}></div>
            <div className="w-32">
              <p>produce</p>
              <p>quantity</p>
            </div>
